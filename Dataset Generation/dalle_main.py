...
def save_json_with_check(json_path, caption_data):
    try:
        if os.path.exists(json_path):
            with open(json_path, 'r') as json_file:
                if os.path.getsize(json_path) > 0:
                    ...
                    if existing_data:
                        print(f"[INFO] 🔍 JSON already exists and is valid at {json_path}. Skipping regeneration...")
                        return
        with open(json_path, 'w') as json_file:
            json.dump(caption_data, json_file)
            print(f"[INFO] 💾 JSON saved to {json_path}")
    except Exception as e:
        print(f"[ERROR] ❌ Error saving JSON file at {json_path}: {e}")
        try:
            with open(json_path, 'w') as json_file:
                json.dump(caption_data, json_file)
                print(f"[INFO] 💾 JSON regenerated and saved to {json_path}")
        except Exception as e:
            print(f"[ERROR] ❌ Failed to regenerate and save JSON at {json_path}: {e}")

def process_image_pair(image_path, mask_paths, crop_paths, save_path, processor, model, actions):
    try:
        ...
        with Image.open(BytesIO(response.content)) as img:
            ...
            save_json_with_check(json_path, caption_data)
...
def process_folders(levels, data_folder, mask_folder, crop_folder, output_folder, processor, model):
            ...
            for level in levels:
                ...
                try:
                    if len(existing_mask_paths) >= len(actions):
                        ...
                        process_image_pair(base_image_path, existing_mask_paths[:len(actions)], existing_crop_paths[:len(actions)], output_path, processor, model, actions)
                    else:
