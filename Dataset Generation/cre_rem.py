    ...
    for crop_path, action in zip(crop_paths, actions):
        try:
            if os.path.exists(crop_path):
                ...
                masked_image_inputs = processor(images=crop, return_tensors="pt").to(device)
...
def process_image_pair(image_path, mask_paths, crop_paths, save_path, processor, model, actions):
    try:
        ...
        with Image.open(BytesIO(response.content)) as img:
            ...
            captions_folder = "captions_cer_rep"
...
def process_folders(levels, data_folder, mask_folder, crop_folder, output_folder, processor, model):
    ...
    actions_dict = {
        ...
        "removal": ["removal"],
        "creation": ["creation"],
        "comb1": ["removal", "replacement"],
        "comb2": ["removal", "creation"],
        "comb3": ["replacement", "creation"],
        "comb4": ["removal", "replacement", "creation"]
    }
